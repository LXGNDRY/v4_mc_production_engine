...
class V4EngineIntegrated:
    ...
    def _apply_hbu(self, market, mu, sigma):
        return mu, sigma
    def _p_hit_raw(self, market, mu, sigma, line, direction, consensus=None):
        import math
        sg=max(1e-9,float(sigma)); z=(line-float(mu))/sg
        from math import erf, sqrt
        p_under = 0.5*(1.0+erf(-z/math.sqrt(2)))
        return 1.0-p_under if str(direction).lower().startswith("o") else p_under
